...
def main():
    ...
    # 检查是否已有 features.json
    features_file = component_dir / "features.json"
    if features_file.exists():
        print(f"⚠️  组件已初始化（features.json 已存在）")
        print(f"   如需重新生成，请先删除: {features_file}\n")
        return
    
    # 检查 spec.md 是否存在
    spec_file = component_dir / "spec.md"
    if not spec_file.exists() and not args.spec:
        ...
        print()
        print("请按以下步骤操作:")
        print(f"  1. 创建规格文档: vim {spec_file}")
        print(f"  2. 重新运行初始化")
        print()
        print("💡 提示: 可以使用 Cursor AI 帮助生成规格文档")
        print()
        return
    ...
    # 默认使用 AI 辅助生成
    print("🤖 生成 AI 提示词（从 spec.md 生成 features.json）...")
    ...
    print("=" * 70)
    print()
    print("方式1: 使用 pep-dev 命令（推荐）⭐")
    print(f"  pep-dev spec {args.component}")
    print()
    print("方式2: 手动操作")
    print("  1️⃣  在 Cursor 中打开提示词:")
    print(f"     {prompt_file}")
    print()
    print("  2️⃣  使用 Cursor AI 生成功能清单:")
    print("     • Cmd+I (Composer) 或 Cmd+L (Chat)")
    print("     • 粘贴提示词内容")
    print()
