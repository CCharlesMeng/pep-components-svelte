<script lang="ts">
    import { createFallbackRemoteContent, extractArticleContent, getNextStepIndex, getPrevStepIndex, shouldHijackLink } from '../utils/phase2';
    ...
    let remoteContentEl = $state<HTMLDivElement | null>(null);
    let scopedRemoteCss = $derived(scopeRemoteCss(remoteContentData?.css ?? ''));
    ...
    let showAppDropdown = $derived(
        hasApplications &&
        (activeTab?.applications?.some((app) => (app.title ?? '').trim().length > 0) ?? false)
    );
    ...
    async function loadRemoteContent(): Promise<void> {
        ...
        isLoading = true;
        try {
            const preloadedHtml = activeStep.remoteContent.preloaded?.html;
            const preloadedCss = activeStep.remoteContent.preloaded?.css ?? '';

            const fullHtml = preloadedHtml ?? await fetchWithCache(activeStep.remoteContent.source.htmlUrl);
            const html = extractArticleContent(fullHtml);
            const css = preloadedHtml
                ? preloadedCss
                : (activeStep.remoteContent.source.cssUrl ? await fetchWithCache(activeStep.remoteContent.source.cssUrl) : '');

            remoteContentData = { html, css };
        } catch (error) {
            void error;
            remoteContentData = createFallbackRemoteContent(
                activeTab.title,
                activeStepIndex + 1,
                activeStep.title,
                activeStep.remoteContent.source.htmlUrl,
                {
    ...
    $effect(() => {
        const el = remoteContentEl;
        if (!el) {
            return;
        }
        const clickHandler = (event: Event) => {
            handleContentClick(event as MouseEvent);
        };
    ...
    $effect(() => {
        ...
        const updateCompact = () => {
            ...
            const availableWidth = Math.max(0, rowEl.clientWidth - appWidth - (showAppDropdown ? gap : 0));
    ...
    function handleContentClick(event: MouseEvent): void {
        ...
        if (shouldHijackLink(href) && onOpenExternal) {
            ...
            onOpenExternal(href ?? '', anchor.innerText || sidebar.texts.openExternalDefaultTitle);
        }
    }

    function scopeCssSelectors(selectorText: string, prefix: string): string {
        return selectorText
            .split(',')
            .map((item) => item.trim())
            .filter(Boolean)
            .map((selector) => {
                // keyframes steps should not be prefixed
    ...
    function scopeRemoteCss(css: string): string {
        if (!css.trim()) {
            return '';
        }
        const prefix = '.pep-cloud-deploy-flow-sidebar__remote-content .rich-text';
        return css.replace(/(^|})\s*([^@{}][^{}]*)\s*\{/g, (full, boundary, selectorText) => {
            const scoped = scopeCssSelectors(String(selectorText), prefix);
...
<aside class="pep-cloud-deploy-flow-sidebar" class:is-floating={isFloating}>
        ...
        <div class="pep-cloud-deploy-flow-sidebar__tabs">
            {#each sidebar.tabs as tab, index (`${tab.title}-${index}`)}
                <button
                    ...
                    title={tab.title}
                >
                    <span class="pep-cloud-deploy-flow-sidebar__tab-text">{tab.title}</span>
                </button>
            {/each}
        </div>
        <div class="pep-cloud-deploy-flow-sidebar__tools">
            {#if isFullscreen}
                <button type="button" aria-label="切换到悬浮" title="切换到悬浮" onclick={onFloat}>
                ...
                >
                    ...
                    {:else}
                        <svg viewBox="0 0 24 24" aria-hidden="true" fill="none">
                            <path d="M7 4h10a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H7" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M11 8l-4 4 4 4" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                    {/if}
                </button>
            {:else}
                ...
                <button type="button" aria-label="折叠" title="折叠" onclick={onCollapse}>
    ...
    {#if currentSteps.length > 0}
        ...
        >
            {#if showAppDropdown && activeTab?.applications}
                <div class="pep-cloud-deploy-flow-sidebar__app-select-wrap" bind:this={appDropdownEl}>
                    <button
                        type="button"
                    ...
                    >
                        ...
                        <span class="pep-cloud-deploy-flow-sidebar__app-trigger-arrow" aria-hidden="true">
                    ...
                    {#if appDropdownOpen}
                        <div class="pep-cloud-deploy-flow-sidebar__app-dropdown" role="listbox">
            ...
            >
                {#each currentSteps as step, index (`${step.title}-${index}`)}
                    <li class:active={activeStepIndex === index}>
                        <button type="button" onclick={(event) => handleStepButtonClick(event, index)}>
                            <span class="pep-cloud-deploy-flow-sidebar__step-dot-hitbox">
                                <Tooltip content={step.title}>
                                    <span class="dot" class:is-completed={index < activeStepIndex}>
                                        {#if index < activeStepIndex}
                                            {#if sidebar.icons?.stepCheckedIcon}
                                                <img src={sidebar.icons.stepCheckedIcon} alt="" />
                                            {:else}
                                                <span class="check-fallback">✓</span>
                                            {/if}
                                        {:else}
                                            {index + 1}
                                        {/if}
                            ...
                            <span class="pep-cloud-deploy-flow-sidebar__step-label" title={step.title}>
                                {step.title}
                            </span>
                        </button>
                        {#if index < currentSteps.length - 1}
                            <span class="line" class:is-completed={index < activeStepIndex}></span>
                        {/if}
                    </li>
                {/each}
            </ol>
    ...
    <div class="pep-cloud-deploy-flow-sidebar__remote-content remote-content" bind:this={remoteContentEl}>
        {#if isLoading}
            <p class="loading">{sidebar.texts.remoteLoadingText}</p>
        {:else if remoteContentData}
            {#if scopedRemoteCss}
                <style>{scopedRemoteCss}</style>
            {/if}
            <div class="rich-text">
...
<style>
    ...
    .pep-cloud-deploy-flow-sidebar__steps .dot {
        width: 16px;
        height: 16px;
        border-radius: 50%;
        display: inline-flex;
    ...
    .pep-cloud-deploy-flow-sidebar__steps .dot.is-completed {
        border: unset;
    }

    .pep-cloud-deploy-flow-sidebar__steps .dot img {
        width: 16px;
        height: 16px;
        object-fit: contain;
    }

    .pep-cloud-deploy-flow-sidebar__steps .dot .check-fallback {
        font-size: 11px;
        line-height: 1;
        font-weight: 700;
    }

    .pep-cloud-deploy-flow-sidebar__steps li.active .dot {
        background: #1f2329;
        border-color: #1f2329;
        color: #fff;
    }
    ...
    :global(.pep-cloud-deploy-flow-sidebar__remote-content img),
    :global(.pep-cloud-deploy-flow-sidebar__remote-content video) {
        max-width: 100%;
        height: auto;
        display: block;
        box-sizing: border-box;
    ...
    :global(.pep-cloud-deploy-flow-sidebar__remote-content iframe) {
        max-width: 100%;
        box-sizing: border-box;
    }

    :global(.pep-cloud-deploy-flow-sidebar__remote-content .pep-cloud-deploy-flow-sidebar__table-wrap) {
        max-width: 100%;
        overflow-x: auto;
        margin: 1em 0;
    ...
    :global(.pep-cloud-deploy-flow-sidebar__remote-content .pep-cloud-deploy-flow-sidebar__table-wrap table) {
        box-sizing: border-box;
    }
    ...
    .pep-cloud-deploy-flow-sidebar__steps.is-compact .dot {
            width: 8px;
            height: 8px;
            min-width: 8px;
            border-width: 1px;
    ...
    .pep-cloud-deploy-flow-sidebar__steps.is-compact .dot.is-completed {
    ...
    .pep-cloud-deploy-flow-sidebar__steps.is-compact .dot.is-completed .check-fallback {
            display: none;
    }

    .pep-cloud-deploy-flow-sidebar__steps.is-compact li.active .dot {
            border-color: #2375f3;
            background: #2375f3;
            box-shadow: none;
    }
