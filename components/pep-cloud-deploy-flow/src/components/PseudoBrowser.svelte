<script lang="ts">
    ...
    let defaultTabTitle = $derived(
        iframePages.newTabPage.tabTitle,
    );
    let pageTitleText = $derived(
        iframePages.newTabPage.title.text,
    );
    let pageTitleIcon = $derived(iframePages.newTabPage.title.icon);
    let shortcutTitle = $derived(
        iframePages.newTabPage.shortcuts.title,
    );
    let shortcutItems = $derived(
        iframePages.newTabPage.shortcuts.items,
    );
    let addressPlaceholder = $derived(
        iframePages.newTabPage.addressPlaceholder,
    );
        ...
        <div class="pep-cloud-deploy-flow-browser__address-wrap">
            <label class="pep-cloud-deploy-flow-browser__address-bar">
                {#if iframePages?.icons?.headerLogo}
                    ...
                    />
                {:else}
                    <div class="pep-cloud-deploy-flow-browser__logo-dot pep-cloud-deploy-flow-browser__address-prefix-logo"></div>
                {/if}
                <input
                    type="text"
        ...
        {:else}
            ...
            >
                <div class="pep-cloud-deploy-flow-browser__blank-title">
                    ...
                    {:else}
                        <span
                            class="pep-cloud-deploy-flow-browser__blank-title-dot"
                            >i</span
                        >
                    {/if}
                    <h3>{pageTitleText}</h3>
                ...
                <div class="pep-cloud-deploy-flow-browser__shortcut-group">
                    ...
                    {#if shortcutItems.length}
                        <div
                            class="pep-cloud-deploy-flow-browser__shortcut-list"
                        >
