<script lang="ts">
    ...
    let { cardData = {}, cardBackgroundColor = "#ffffff" } = $props<{
        cardData?: CardData;
...
<div class="card" style="background-color: {cardBackgroundColor};">
    {#if (cardData.showImage !== false) && cardData.imageUrl}
    ...
    <div class="card-body">
        {#if (cardData.showTitle !== false) && cardData.title}
            <h3 class="card-title">{cardData.title}</h3>
        {/if}

        {#if (cardData.showDescription !== false) && cardData.description}
        ...
        <div class="card-footer">
            ...
            {#if cardData.showInquiry !== false}
                <button class="inquiry-btn">Inquiry</button>
            {/if}
        </div>

        {#if cardData.linkUrl}
            <a href={cardData.linkUrl} class="card-link" aria-label="View product"></a>
        {/if}
    </div>
</div>
