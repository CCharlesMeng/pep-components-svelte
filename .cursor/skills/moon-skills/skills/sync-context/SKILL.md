...
- 负责承接 `audit` post-ship reflect 路由出的 context 更新
- 不负责首次初始化
- 不负责事故调试
...
2. **不要无触发地更新上下文。** 任何更新都必须对应代码、结构、共享模式、特性变化或已明确路由的 reflect 结论。
3. **不要静默重写稳定内容。** 能保留的部分尽量保留。
4. **不要让过期上下文继续看起来很可靠。** confidence 下降时必须明确写出。
5. **不要把纯观察当成上下文更新。** 只有被明确路由为持久 context 的结论才写回资产。
6. **不要把 sync 当 initialize 用。** 若没有 `profile.yaml`，先走 `initialize`。
...
- `audit` 的 post-ship reflect 明确指出需要回写 context 资产
...
- `retro`
  - 只承接 `audit` 已明确路由的 reflect 结论
  - 可复用模式、反例和风险热点写回 `references.yaml`
  - 关键特性的 code / test / spec refs 变化写回 `features/index.yaml`
  - 人类可读摘要与本轮 context 状态写回 `PROFILE.md`
  - 只有当交付显示模块边界或关键 flow 变化时，才更新 `profile.yaml`
- `manual`
  - 明确说明人工指定了哪些更新范围

如果同步导致某些判断不再可靠，必须降低 `confidence` 或增加 `uncertainty`。

纯观察但不值得落盘的 reflect 结论，应保留在 `audit` 输出里，不要硬写进 `.project-context`。
...
完整对接说明见 [references/github-context-check.md](references/github-context-check.md)。
...
- 若触发源为 `retro` / `post-ship`，说明 reflect 结论被路由到了哪里
...
- 哪些 reflect 观察刻意未写入 `.project-context`
